...
export default function Quiz() {
  ...
  if (showResults) {
    return (
      <div className="bg-white p-6 md:p-8 rounded-lg shadow-xl w-full max-w-2xl text-center">
        <h2 className="text-2xl font-bold text-gray-800 mb-4">¡Cuestionario Completado!</h2>
        {score >= 33 ? (
          <p className="text-2xl font-bold text-green-600 mb-4">¡APROBADO! 🎉</p>
        ) : (
          <p className="text-2xl font-bold text-red-600 mb-4">¡REPROBADO! 😞</p>
        )}
        <p className="text-xl text-gray-700 mb-6">
          Tu puntaje final es: <span className="font-bold">{score}</span> de 38
        </p>
        <button onClick={startNewQuiz} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded-lg transition duration-200 ease-in-out inline-block mt-4">
          Intentar con otras 35 preguntas
        </button>
  ...
  return (
    <div className="bg-white p-6 md:p-8 rounded-lg shadow-xl w-full max-w-2xl relative">
      ...
      <h1 className="text-2xl md:text-3xl font-bold text-center text-gray-800 mb-6">Cuestionario Licencia Clase B (Aleatorio)</h1>
      ...
      <div id="quiz-container">
        ...
        <div className="mt-4 text-center font-medium h-6">
          {isAnswerChecked && (
            <p className={score > 0 ? 'text-green-600' : 'text-red-600'}>
